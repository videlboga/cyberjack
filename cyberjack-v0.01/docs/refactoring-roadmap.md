...
**Частично выполнен.** Проведена инвентаризация периодических процессов. Создана единая очередь фоновых заданий; память и автономные сцены переведены н...
...
**Критерии закрыты:** `selectedMemoryBlocks` в `prompt.build` trace.
