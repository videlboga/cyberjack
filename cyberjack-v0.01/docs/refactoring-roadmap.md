...
**Частично выполнен.** Проведена инвентаризация периодических процессов. Создана единая очередь фоновых заданий; память и автономные сцены переведены на неё. `timeFlow` — единственный heartbeat-драйвер (см. правило 3.4).

**Открытый критерий:** «два персонажа в одной доступной сцене могут знакомиться и обмениваться репликами через стандартный чат» — `socialTransactions`/`social_initiative` реализованы, но нет явного теста, подтверждающего сценарий.
...
**Частично выполнен.** Проведена инвентаризация prompt/context builders и проактивных путей. Введён контракт `CharacterStimulus` → `CharacterTurnContext` и единый executor `executeCharacterSpeech`.

**Открытый критерий:** «реплика всегда учитывает сообщения, появившиеся после предыдущего импульса» — не реализован (нет `afterMessageId`/`sinceMessageId` в `characterTurnContext`).
...
**Частично выполнен.** Проведена инвентаризация адаптеров. Fallback-логика существует; добавлены кеш эмбеддингов и общий trace/request ID.

**Открытый критерий:** «декларативная конфигурация назначений моделей» (по приоритетам/таймаутам/попыткам/форматам) — не реализован, остаётся env-based.
...
**Частично выполнен.** Инвентаризация модели памяти и перевод коррекции на неблокирующую очередь (`episode.intervene`).

**Открытые критерии:** «происхождение тегов», «подсветка связанных фрагментов», «отображаемая хроника» — не реализованы.
...
**Открытые критерии:** «выбранные блоки памяти в prompt», «профилирование после измерения» — не реализованы.
