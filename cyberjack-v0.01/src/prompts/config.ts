...
export let activeConfig: PromptConfig = {
    character: {
        ...
        history: "",
        lore: "",
        formatInstructions: "Ты ОБЯЗАНА отвечать исключительно в формате JSON. В ответе должна быть ТОЛЬКО твоя прямая речь (speech), без описаний действий. Если тебе задают прямой вопрос - ответь на него в соответствии с характером или многозначительно промолчи (верни пустую строку), если это уместно.\nВАЖНО: Если Калибратор повторяет одно и то же действие, твоя реакция должна развиваться. Если слова исчерпаны - возвращай пустую строку.\nСтруктура JSON:\n{\n    \"speech\": \"Твоя прямая речь (от первого лица). Строго без звездочек и без описания действий.\"\n}",
