...
export function getLaboratorySpatialContext(characterOrSubjectId: string, playerId = 'PL-1'): SpatialContext | null {
    ...
    const deviceProtocolDescription = occupiedAsset?.asset_id === 'lab_sex_machine' && deviceSession
        ? (() => {
            const running = deviceSession.status === 'running';
            const paused = deviceSession.status === 'paused';
            const anal = String(deviceSession.stimulationMode) === 'anal';
            const parts = [
                anal
                    ? 'ты стоишь на четвереньках на металлической поверхности секс-машины, бёдра разведены, а попа приподнята — поза уязвимая и открытая'
                    : 'ты лежишь на металлической поверхности секс-машины',
                anal
                    ? 'твои руки и ноги зажаты жёсткими фиксаторами, поэтому ты не можешь ни выпрямиться, ни свести бёдра, ни изменить положение тела'
                    : 'твои руки и ноги зажаты жёсткими фиксаторами, поэтому ты не можешь самостоятельно изменить положение тела',
                tickling
                    ? 'ступни закреплены в отдельных держателях, а фиксаторы не дают отдёрнуть ноги'
                    : anal
                        ? 'твой анус выставлен на обозрение, и ты чувствуешь, как прохладный воздух касается открытой кожи'
                        : 'в твоё влагалище введён крупный поршень с рельефной поверхностью',
            ];
            if (running) {
                parts.push(
                    tickling
                        ? 'механические щётки проходят по подошвам, под пальцами и вдоль сводов'
                        : anal
                            ? 'изнутри ты постоянно ощущаешь объём поршня, его давление и движение'
                            : 'изнутри ты постоянно ощущаешь его объём, давление и движение',
                    feltPower,
                    feltRhythm,
                    feltPolicy,
                ...
                // paused и stopped: щётки/поршень не касаются тела, они просто рядом.
                const contact = tickling
                    ? 'щётки стоят рядом с твоими ступнями, не касаясь их — ты видишь их и понимаешь, для чего они предназначены'
                    : 'поршень лежит рядом, не введённый — ты видишь его и понимаешь, для чего он предназначен';
                parts.push(
                    contact,
                    paused
                        ? 'механизм на паузе, хотя твоё тело всё ещё удерживается на месте'
                        : 'механизм остановлен, хотя твоё тело всё ещё удерживается на месте',
                );
            }
            return parts.join('; ') + '.';
        })()
    ...
    const postureDescription = occupiedAsset?.asset_id === 'lab_sex_machine'
        ? String(deviceSession?.stimulationMode) === 'anal'
            ? 'ты стоишь на четвереньках на металлической поверхности, бёдра разведены, попа приподнята; фиксаторы не дают изменить положение'
