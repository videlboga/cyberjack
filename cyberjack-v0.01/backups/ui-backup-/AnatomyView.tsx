    ...
    <div className="anatomy-overlay">
      ...
      <div className="anatomy-panel center-panel">
        ...
        <div className="point-details">
         ...
         {getPointContexts(selectedPoint).map((ctx, idx) => (
           <div key={`${ctx.id || ctx.actionId || 'ctx'}-${idx}`} className="ctx-chip">{ctx.label || ctx.actionId || ctx.type || ctx.id || "Неизвестно"}</div>
         ))}
