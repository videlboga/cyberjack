...
export async function buildPromptPayload(
    ...
    const stateSection = isObserver
        ? \`[Статус: Наблюдатель]\\nТы наблюдаешь за сценой со стороны. Не передавай специальные метки, указывающие, кто является целью воздействия; пусть модель сама решит важность и направленность взаимодействий.\`
    ...
    const voiceInstructions = isObserver
        ? \`\\n- Говори от первого лица.\\n- Ты — НАБЛЮДАТЕЛЬ (зритель). Комментируй происходящее со стороны.\\n- Не добавляй специальные метки или роли участникам; оставь интерпретацию направленности воздействия на усмотрение модели.\`
